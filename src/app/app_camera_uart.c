...
void camera_uart_rx_callback(void)
{
    ...
    if (ret == 0 && len == CAMERA_FRAME_SIZE) {
        ...
        // 保存到历史记录
        uint8_t hist_idx = g_camera_uart.rx_history_index % CAMERA_RX_HISTORY_SIZE;
        memcpy(&g_camera_uart.rx_frame_history[hist_idx], &g_camera_uart.rx_frame, sizeof(camera_frame_data_t));
        g_camera_uart.rx_history_index++;
    } else {
        // 无帧数据或获取失败
        g_camera_uart.last_rx_status = (uint8_t)ret;
    }
}
...
int camera_uart_send_frame(uint8_t mode, uint8_t id, int16_t data_x, int16_t data_y)
{
    ...
    // 发送帧
    ret = uart_send_frame(&frame[2], CAMERA_FRAME_SIZE - 4);  // 去掉帧头帧尾
    ...
    if (ret == 0) {
        ...
        g_camera_uart.last_tx_status = (uint8_t)ret;
        return -1;
    }
}
...
{
    if (frame == NULL || g_camera_uart.rx_frame.valid == 0) {
        return -1;
    }
    
    memcpy(frame, &g_camera_uart.rx_frame, sizeof(camera_frame_data_t));
    g_camera_uart.rx_frame.valid = 0;  // 清除有效标志
...
{
    if (index >= CAMERA_RX_HISTORY_SIZE || frame == NULL) {
        return -1;
    }
    
    memcpy(frame, &g_camera_uart.rx_frame_history[index], sizeof(camera_frame_data_t));
